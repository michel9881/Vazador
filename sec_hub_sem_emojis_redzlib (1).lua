...
-- Carregar RedzLib
local RedzLib = loadstring(game:HttpGet("https://raw.githubusercontent.com/wx-sources/VictoryLib/refs/heads/main/Redzhubui%20(1).txt"))()
...
local function headsitOnPlayer(targetPlayer)
    ...
    if not targetPlayer.Character or not targetPlayer.Character:FindFirstChild("Head") then
        warn("Jogador alvo sem cabea ou personagem.")
        return false
    end
    local targetHead = targetPlayer.Character.Head
    local localRoot = character:FindFirstChild("HumanoidRootPart")
    if not localRoot then
        warn("Seu personagem no tem HumanoidRootPart.")
        return false
    end
...
OpTab:AddButton({
    ...
    Callback = function()
        ...
        if not target then
            warn(" Jogador no encontrado para headsit.")
            return
        end
...
OpTab:AddSection({ "Ver viso de outros jogadores" })
...
local dropdown = OpTab:AddDropdown({
    ...
    Callback = function(value)
        ...
        print(" Jogador selecionado para viso: " .. value)
    end
})

local viewing = false
local toggle = OpTab:AddToggle({
    Name = " Ativar viso do jogador",
    Default = false,
        ...
        if viewing then
            if selectedViewPlayer then
                ...
                if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("Humanoid") then
                    ...
                    print(" Agora vendo a viso de " .. selectedViewPlayer)
                else
                    warn(" Jogador invlido ou sem personagem.")
                end
            else
    ...
    CasasTab:AddButton({
        ...
        Callback = function()
            ...
            if House and game.Players.LocalPlayer.Character then
                ...
                print("Casa no encontrada: " .. tostring(SelectHouse))
            end
        end
    })
end)
        ...
        Callback = function()
            ...
            if House and House:FindFirstChild("HousePickedByPlayer") and game.Players.LocalPlayer.Character then
                ...
                if safe then
                    ...
                    print("Cofre no encontrado na casa: " .. tostring(SelectHouse))
                end
            else
                print("Casa no encontrada: " .. tostring(SelectHouse))
            end
...
local audios = {
    ...
    {Name = "udio Alto", ID = 6855150757},
    {Name = "Rudo", ID = 120034877160791},
...
AudioAllTab:AddTextBox({
    Name = "Insira o ID do udio ou Musica",
...
AudioAllTab:AddParagraph({"Info", "Loop de tocar udio (Todos players do Server ouvem)"})
...
AudioAllTab:AddSection({"Tocar udio"})
...
AudioAllTab:AddParagraph({"Info", "Todos do server ouvem o udio"})
...
local function KillPlayerCouch()
    ...
    if not target or not target.Character then
        warn("Erro: Jogador alvo no encontrado ou sem personagem")
        return
    end

    local char = LocalPlayer.Character
    if not char then
        warn("Erro: Personagem do jogador local no encontrado")
        return
    end
    ...
    if not hum or not root or not tRoot then
        warn("Erro: Componentes necessrios no encontrados")
        return
    end
...
local function BringWithCouch()
    ...
    if not couch then
        warn("Erro: Sof no encontrado no Backpack")
        return
    end
    ...
    if seat1 and seat2 and handle then
        ...
        warn("Erro: Componentes do sof no encontrados")
        return
    end
...
local function KillWithCouch()
        ...
        Callback = function()
            ...
            if killDropdown and #tablePlayers > 0 then
                ...
                if selectedPlayerName and not Players:FindFirstChild(selectedPlayerName) then
                    ...
                    print("Seleo resetada, jogador no est mais no servidor.")
                end
            else
                print("Erro: Dropdown no encontrado ou nenhum jogador disponvel.")
            end
        end
    })
    ...
    Troll:AddButton({
        ...
        Callback = function()
            if not selectedPlayerName or not Players:FindFirstChild(selectedPlayerName) then
                print("Erro: Player no selecionado ou no existe")
                return
            end
            local character = LocalPlayer.Character
            local humanoidRootPart = character and character:FindFirstChild("HumanoidRootPart")
            if not humanoidRootPart then
                warn("Erro: HumanoidRootPart do jogador local no encontrado")
                return
            end
            ...
            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                ...
                print("Erro: Player alvo no encontrado ou sem HumanoidRootPart")
            end
        end
    })
    ...
    local MethodSection = Troll:AddSection({ Name = "Mtodos" })

    Troll:AddDropdown({
        Name = "Selecionar Mtodo para Matar",
        Options = {"Bus", "Couch", "Couch Sem ir at o alvo [BETA]"},
        Default = "",
        Callback = function(value)
            methodKill = value
            print("Mtodo selecionado: " .. tostring(value))
        end
    })
                            ...
                            if targetPlayer and targetPlayer.Character and targetPlayer.Character:FindFirstChild("HumanoidRootPart") then
                                ...
                                if targetHumanoid and targetHumanoid.Sit then
                                    if character.Humanoid then
                                        ...
                                        print("Player voltou para a posio inicial.")
                                    end
                                    break
                                else
        ...
        if Check() then
            ...
            if not House then
                ...
                if #availableHouses > 0 then
                    ...
                    print("Nenhuma casa disponvel para compra!")
                    return
                end
            end
...
Troll:AddButton({
    ...
    Callback = function()
        if not selectedPlayerName or not game.Players:FindFirstChild(selectedPlayerName) then
            warn("Nenhum jogador selecionado ou no existe")
            return
        end
        ...
        if not Humanoid or not RootPart then
            warn("Humanoid ou RootPart invlido")
            return
        end
        ...
        if not TargetRP or not TargetH then
            warn("Humanoid ou RootPart do alvo no encontrado")
            return
        end
...
Troll:AddButton({
    ...
    Callback = function()
        ...
        print("Spin e foras removidas do jogador.")
        ...
        print("Jogador liberado, seguro na posio.")
    end
})
...
DiversosTab:AddSection({"Funes Diversas"})
...
Fun2Tab:AddSection({"Configuraes de Player"})
...
TrollTab:AddButton({
    Name = "[] Expanso de Domnio",
    Description = "Ativa a Expanso de Domnio OP",
    Callback = function()
...
TrollTab:AddButton({
    Name = "[] Cancelar Expanso de Domnio",
    Description = "Para a Expanso de Domnio",
    Callback = function()
